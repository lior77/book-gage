...
**ומארח אחד שאינו ברשימה ולא יהיה: ‏`www.idealista.pt`.** הוא חסום לסביבה
הזאת ב-DataDome (403 לכל קריאה), והוא **אינו נדרש**: המודעות מגיעות דרך
המחבר הרשמי של idealista, שהוא כלי MCP ולא מארח שיוצאים אליו. פתיחת רמת
הרשת לא תעזור כאן, וגרידה של האתר נגד התנאים שלו (`IDEALISTA.md` §8).
האפליקציה עצמה כן **פותחת** קישורים ל-`www.idealista.pt` בדפדפן — זו פעולת
המשתמש, לא קריאת רשת של האפליקציה, והיא מוצהרת ברשימת ONLINE שבעמוד ״מה דורש
רשת״.

**עדכון 2026-09-19:** ‏`snig.dgterritorio.gov.pt` **עונה 200** — הרשימה שלמטה
סימנה אותו כנופל וזה כבר לא נכון. נבדקו באותה הרצה גם `cartografia.dgterritorio.gov.pt`
(200) ו-`cdd.dgterritorio.gov.pt` (307, חי): שלושתם הדרך אל **ה-MDT מ-LiDAR** שיחליף
את גובה ושיפוע שנמחקו ב-2.8.0. ההורדה עצמה דורשת הרשמה חינם, ולכן היא מעשה של
המשתמש ולא של הסביבה. **ומארח חדש שהאפליקציה פונה אליו:** `tile.opentopomap.org`,
רקע קווי הגובה — מוצהר ברשימת ONLINE שבעמוד ״מה דורש רשת״.
