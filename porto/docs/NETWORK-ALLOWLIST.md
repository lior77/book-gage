...
הסביבה שבה נכתב המסמך הזה אכן חסמה הכול. הסביבה שרצה היום לא: מתוך 17 המארחים
שהסקריפט בודק, עשרה עונים, וביניהם `www.ine.pt` — המקור בעדיפות הראשונה בכל
`DATA-REQUEST.md`.
...
**עדיין שווה להעלות ל-`Full` או ל-`Custom`:** `overpass-api.de`, `www.dgadr.gov.pt`
ו-`snit.dgterritorio.gov.pt` ממשיכים ליפול, ו-`geo2.apambiente.pt` נדחה בפירוש
במדיניות (‏502 ל-CONNECT). הרשימה שלמטה נשארת נכונה — היא פשוט כבר לא תנאי לכל
משיכת נתונים, אלא רק למה שנשאר.
...
נפתח GitHub בלבד; בסביבה של 2026-09-07 נפתחים עשרה מתוך שבעה-עשר גם בלי
לשנות דבר. ראה את פסקת העדכון בראש הקובץ לפני שמסיקים מהמספר הזה משהו.
