...
> **‏2026-09-14: מכולה חדשה לא מגיעה עם אף אחת מהן, ואין במאגר קובץ דרישות.**
> ‏`build.py` נופל מיד על `ModuleNotFoundError: geographiclib`. הרשימה למעלה
> תיארה מכולה מסוימת, לא את המאגר. בפתיחת סשן במכולה נקייה:
>
> ```bash
> pip install shapely pyproj pyshp openpyxl geographiclib
> cd "$PORTO" && PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1 npm install --no-save playwright@1.49.1
> ```
>
> ‏`sqlite3` מגיע עם פייתון. ‏`node_modules/` כבר ב-`.gitignore`.
...
### שני הנתיבים שעדיין לא נבדקו

המשתמש אמר במפורש: **״לגבי שני הכפתורים הנוספים לא בדקתי תקינות הזרימה״** —
כלומר **ממפה** ו**מכתובת**. האישור מ-2026-09-14 אינו נוגע בהם, ואין עליהם
דיווח — לא חיובי ולא שלילי.
...
| 3 | **שני נתיבי הבחירה שלא נבדקו** | ‏§7. ״ממפה״ ו״מכתובת״ — אין עליהם דיווח. ״מתמונה״ אושר ב-2026-09-14 |
