...
ROLES = {
    ...
    "--map-region": "הכתום של שני אזורי NUTS III — הקו, והמילוי ב-40%",
...
PATTERNS = [
    ...
    ("mapctl", "פקד צף על המפה",
     ...
     ["mt", "menu-btn", "reset-btn", "menu-x", "crumb", "now", "one",
      "pick-bar", "menu-top", "menu-top-b", "menu-lang"]),
    ("regions", "שכבת האזורים",
     "שני אזורי NUTS III כשכבה נבחרת: מסך משלהם, מפתח שנלחץ, והנבחר נצבע "
     "כתום ב-40%.",
     ["region-row", "lbl-region"]),
    ("climate", "מסך האקלים",
     "שתי תחנות IPMA כשתי נקודות ושני כרטיסים — לא צביעה של יחידה. הנבחרת "
     "פורסת טבלה של שנים־עשר חודשים, כי הערך הוא המספר ולא גובה של עמודה.",
    ...
    ("docbar", "שורת התצוגה",
     "רצועה קבועה בראש חצי הטקסט לפקדים של המוד עצמו — רשימה/מורחב. היא אינה "
     "נגללת עם הטקסט, ואינה מנווטת: המודים נבחרים בתפריט.",
     ["docbar"]),
    ("listings", "מוד נכסים",
     "מודעות כציטוט: מחיר, תמונה וכיתוב הספק, בלי צ׳יפ על שום מספר; הטופס בטור "
     "אחד; והמודעה השמורה עם רצועת ״עותק שמור״ על כל תמונה.",
     ["lst", "lst-", "lf-", "ph-cap"]),
    ("conf", "סיווג הוודאות",
     "שלוש המילים של כלל 3 כשלושה צ׳יפים — מאומת, מדווח, מחושב כאן — וכל אחד "
     "נושא את המילה ולא רק את הצבע. שדה שאינו מספרי אינו נושא צ׳יפ כלל.",
     ["conf", "conf-"]),
    ("filter", "מסך הסינון",
     "תנאי הוא שדה, יחס וערך, ונקרא כמשפט בשורה אחת. התשובה היא **שתי** שורות "
     "באותו גודל: כמה עונות, וכמה לא נבדקו כלל — יחידה בלי נתון אינה יחידה "
     "שנפלה בתנאי.",
     ["flt-", "flt", "is-zero"]),
    ("series", "קו לאורך הרבעונים",
     "שתי סדרות INE לאורך 26 רבעונים, בקו אחד לכל סוג דיור: הקו נקטע היכן "
     "שרבעון לא פורסם, נקודה בודדת בין שני חורים מקבלת עיגול משלה, ואין "
     "בכרטיס הזה שום הפרש בין רבעונים — החלונות חופפים.",
     ["ser-", "ser"]),
    ("compare", "מסך השוואת נתונים",
     "שדה אחד על כל יחידות הרמה: מפתח צבעים, רשימה מדורגת, ומחליף שדה.",
     ["cmp-"]),
    ("cons", "מסך מגבלות בנייה",
     "אינדקס הצבעים מעל הכותרת, בר ההורדה שקורה פעם אחת, וארבעת המספרים — "
     "שהרביעי בהם הוא האיחוד ולא הסכום.",
     ["cons-", "crow", "crow-", "crows"]),
    ("crus", "ייעוד קרקע",
     "שני מעמדות הקרקע של חוק התכנון הפורטוגלי והקטגוריות שבתוכם — אותה שורה "
     "של מסך המגבלות, בדרגה אחת שקטה יותר.",
     ["crow-sub"]),
    ("layers", "לוח השכבות",
     "הלוח הנפתח מתוך התפריט, ובתוכו מתגי השכבות, החגורות, "
     "והקישור לרשומת המקור של שכבה — שורה משל עצמו, משום שכפתור אינו יכול "
     "להכיל כפתור.",
     ["panel", "panel-h", "lay", "lay-", "belt", "belt-sw", "srcln"]),
    ("places", "המקומות שלי",
    ...
    ("link", "נכס שממתין לפרטיו",
     "קישור שהודבק הוא סימנייה שיודעת שהיא סימנייה: קוד, דרך אל המקור, "
     "והמשפט שצריך להדביק בצ׳אט — והמשפט עצמו על המסך וניתן לסימון, כי כפתור "
     "העתקה שנכשל בשקט גרוע מאין כפתור. אין בה מחיר, שטח או נקודה על המפה, "
     "משום שאין לה.",
     ["lnk", "lnk-"]),
    ("photo", "תמונות",
...
""" % {
        ...
        "lnw": "".join(
            ...
            for k, he, role in (
                ("region", "region — שני אזורי NUTS III (שכבה נבחרת)",
                 'כתום <code dir="ltr">%s</code> — רק במסך ״אזורים״ שנפתח מהתפריט'
                 % lines["region"]),
